...
# The outer edge (away from the road) ramps down from the road-flush inner edge
# toward the terrain instead of ending in a sharp _RAISE-high cliff that cars
# catch on. It sits a hair *above* terrain (not below) so the sidewalk strip
# stays complete and visible across its full width — never buried by the terrain
# mesh — while the gentle +_RAISE → +_OUTER_RAISE drop keeps it cliff-free. The
# small positive offset also clears bilinear interpolation bleed (z-fighting).
_OUTER_RAISE = 0.05  # metres the outer edge rides above terrain
